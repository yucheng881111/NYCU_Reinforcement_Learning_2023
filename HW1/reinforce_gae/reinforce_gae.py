...
class Policy(nn.Module):
    ...
    def __init__(self):
        ...
        self.Layer2 = nn.Linear(self.hidden_size, self.action_dim)     # policy output
        self.Layer3 = nn.Linear(self.hidden_size, self.hidden_size2)                   
        self.Layer4 = nn.Linear(self.hidden_size2, 1)                  # value output             
        ########## END OF YOUR CODE ##########
    ...
    def forward(self, state):
        ...
        x = self.Layer1(state)
        #x = F.relu(x)
        policy_logits = self.Layer2(x)
        action_prob = Categorical(logits=policy_logits)
        
        x = self.Layer3(x)
        x = F.relu(x)
        state_value = self.Layer4(x)
        ########## END OF YOUR CODE ##########
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        for r in reversed(self.rewards):
            ...
            returns.append(R)
        returns.reverse()
        #returns = self.standardization(returns)
        ...
        for i in saved_actions:
            ...
            values = torch.cat((values, i.value))
        
        values = values.squeeze()
        advantages = self.gae(self.rewards, values)
        advantages = torch.Tensor(advantages)
        advantages = advantages.to(device)
        # negative because of gradient ascend
        policy_loss = -(advantages * a_prob).sum()
